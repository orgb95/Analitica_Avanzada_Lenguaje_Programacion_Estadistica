...
paquetes <- c("readr", "randomNames", "lubridate", "ggplot2", "dplyr", "readxl", "tidyr")
...
# Convertir la columna 'Fech' a formato Date con año-mes-dia
gallo$Date <- as.Date(gallo$Date, format = "%Y-%m-%d")

str(gallo$Date) #Comprobando la transformacion a Date
...
## Base Empleados ----
#Empleado <- read_xlsx("\\Users\\jessu\\Desktop\\Maestria\\R CLASE\\data_anom\\Empleado.xlsx")
#dim(Empleado);
#head(Empleado);
...
gallo$fecha_ingreso <- ymd_hms(gallo$FechaIngresoCliente)
...
# 4.2 -  Catalogo Empleado: ----

# Garantizando que no varien los nombres a generar
#set.seed(123)
#Empleado$NomEmpleado <- randomNames(n = nrow(Empleado))

#Verificando empleado duplicados
#duplicados <- sum(duplicated(Empleado$NomEmpleado))
#duplicados


# Añadir nombres del Empleado aleatoriamente al gallo 'gallo'
#gallo <- merge(gallo, Empleado, by = "Sucursal", all.x = TRUE)
...
#gallo <- subset(gallo, select = -c(ID_empleado, Id_Sucursal, Nombre, Antigüedad, Capacitacion, NomEmpleado))

#c_empleado <- gallo %>%
# select (ID_empleado,NomEmpleado,Sucursal,Capacitacion,Antiguedad) %>%
#distinct()
...
#gallo <- gallo %>%
#group_by(Sucursal) %>%
# mutate(NomEmpleado = Empleado$NomEmpleado(90)) %>%
# ungroup()

# 4.3 - Catalogo TipoCliente: ----
...
#Generano la descripcion de gama o tipo de productos.
...
# 4.6 - Catalogo Producto: ----

ProductosGamaBlanca <- read.csv2("C:\\Users\\oscar\\OneDrive\\Escritorio\\Proyecto Fin de Curso\\Datos\\ProductosGamaBlanca.csv", sep = ",")
ProductosGamaMarron <- read.csv2("C:\\Users\\oscar\\OneDrive\\Escritorio\\Proyecto Fin de Curso\\Datos\\ProductosGamaMarron.csv", sep = ",")
ProductosGamaGris <- read.csv2("C:\\Users\\oscar\\OneDrive\\Escritorio\\Proyecto Fin de Curso\\Datos\\ProductosGamaGris.csv", sep = ",")
...
Marcas <- read.csv2("C:\\Users\\oscar\\OneDrive\\Escritorio\\Proyecto Fin de Curso\\Datos\\Marcasproducto.csv", sep = ",")
...
# 4.9 - Catalogo Canal Distribución:----
...
Canal <- c("Tiendas minoristas", "Ventas por catálogos y telemarketing",
           "Distribuidoras y Mayorista", "Ferias y eventos especiales",
           "Venta en linea o comercio electrónico")
...
clientes_atendidos_anualmente <- Tabla_Central %>%
  ...
  distinct(Id_Cliente, Fecha) %>%
  left_join(c_tiempo, by = "Fecha") %>%
  select(Id_Cliente, anio) %>%
  distinct(Id_Cliente, anio) %>%
  group_by(anio) %>%
  summarise(n = n())
...
Nuevos_Clientes_Anio <- Tabla_Central %>%
...
Porcentaje_Clientes_Nuevos <- Nuevos_Clientes_Anio %>%
  mutate(anio,
         Clientes_Nuevos = n,
...
Productos_devueltos_anio <- Tabla_Central %>%
  filter(Cantidad_Devuelta > 0)%>%
...
Total_Productos_Vendidos <- sum(Productos_devueltos_anio$Total_Unidades_Vendidas)

Tasa_De_Devolucion_por_producto <- Productos_devueltos_anio %>%
  group_by(Id_Producto, anio) %>%
  mutate(TasaDevolucion = (Total_Cantidades_Devueltas/Total_Productos_Vendidos)*100)

Tasa_De_Devolucion_global_anual <- Productos_devueltos_anio %>%
  ungroup()%>%
  group_by(anio) %>%
...
#### RMarkdown llega hasta acá, esperar actualización del RShiny
...
# Agrupar por año y sucursal
resumenTotal <- Tabla_Central %>%
  group_by(Id_Sucursal, Año = lubridate::year(Fecha)) %>%
  summarise(
    Venta_Total = sum(Venta_Total, na.rm = TRUE),
    Costo_Venta = sum(Costo_Total, na.rm = TRUE),
    Venta_Promedio = mean(Cantidad_Total, na.rm = TRUE),
    Venta_Max = max(Cantidad_Total, na.rm = TRUE),
    Venta_Min = min(Cantidad_Total, na.rm = TRUE),
    Clientes = n_distinct(Id_Cliente),
    Nuevos_Clientes = n_distinct(Id_Cliente[lubridate::year(FechaIngresoCliente) == Año]),
    Devolución = sum(Cantidad_Devuelta, na.rm = TRUE),
    Descuento = sum(Descuento, na.rm = TRUE)
  ) %>%
  ungroup()
...
ResumenZonas <- c_zona %>%
  select(-NombreZona) %>%  
  rename("Número de la zona"= Id_Zona, "Nombre de la zona" = zonas_ventas)



library(dplyr)

# Agrupamos por Id_Cliente y luego calculamos la suma de Venta_Total para cada grupo
Tabla_SumVentas <- Tabla_Central %>%
  group_by(Id_Cliente) %>%
  summarise(VentaTotal = sum(Venta_Total, na.rm = TRUE),
            VentaNetaTotal = sum(Venta_Neta, na.rm = TRUE),
            CantidadTotalProductos = sum(Cantidad_Total, na.rm = TRUE),
            CantidadDevueltaTotal = sum(Cantidad_Devuelta, na.rm = TRUE))
...
Tabla_Central <- select(Tabla_Central, -Year)